...
contract VCPackIssuer is ERC1155, Ownable {
    ...
    event PackCreated(uint256 indexed packId, string name, string description, uint256 milestoneCount);
    event MilestoneSubmitted(address indexed holder, uint256 indexed packId, uint256 milestoneIndex, string proofCID);
    event MilestoneApproved(address indexed holder, uint256 indexed packId,  uint256 milestoneIndex, string feedback, uint256 newProgress);
    event MilestoneRejected(address indexed holder, uint256 indexed packId, string feedback, uint256 milestoneIndex);
    event PackMinted(address indexed holder, uint256 indexed packId);
    ...
    function createPack(
        ...
        newPack.name = _name;
        newPack.description = _description;
        ...
        emit PackCreated(currentPackId, _name, _description, _milestones.length);
    }
    ...
    function approveMilestone(
        ...
        emit MilestoneApproved(_holder, _packId, _milestoneIndex, feedback, holderProgress[_holder][_packId]);
    }
    ...
    function rejectMilestone(
        ...
        emit MilestoneRejected(_holder, _packId, feedback, _milestoneIndex);
    }
    ...
    function submitMilestone(
        ...
        Pack storage pack = packs[_packId];
        require(pack.id != 0, "Pack does not exist"); //check
        require(_milestoneIndex < pack.milestones.length, "Invalid milestone index");
    ...
    function mintCompletedPack(
        ...
        emit PackMinted(_msgSender(), _packId);
    }
